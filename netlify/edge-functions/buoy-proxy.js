...
export default async () => {
  ...
  try {
    ...
    if (!resp.ok) {
      return new Response(
        JSON.stringify({ error: true, reason: `Upstream HTTP ${resp.status}` }),
    ...
    // Normalise field names — AODN uses verbose CF standard names
    const out = {
      source: 'AODN WFS — Albany Waverider WMO 56011',
      time: feat.time ?? feat.TIME ?? null,
      hs:   feat.hs
